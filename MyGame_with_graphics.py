...
import MyGame_artist as MyGame_graphics
    ...
    def perform_action(self):
        ...
        if not self.is_busy:
            ...
            if len(nearby_agents) > 0 and (self.messaging_cool_down_counter == AGENT_MESSAGING_COOL_DOWN_PERIOD) \
                ...
                del board.pieces[str(box)]
                board.canvas.delete(str(box))
    ...
    def perform_action(self) -> bool:
        ...
        if len(nearby_targets) > 0:
            ...
            if type(target) is Hub:
                ...
                del board.pieces[target.message_handle]
                agents_in_game.remove(target)
                board.canvas.delete(target.message_handle)
...
def main():
    ...
    root = tk.Tk()
    board = MyGame_graphics.GameBoard(root, GRID_SIZE, GRID_SIZE)
    board.pack(side="top", fill="both", expand="true", padx=2, pady=2)

    # setup images
    agent_im = Image.open("agent.png")
    agent_im = ImageTk.PhotoImage(agent_im)
    hub_im = Image.open("hub.png")
    hub_im = ImageTk.PhotoImage(hub_im)
    game_box_im = Image.open("briefcase.png")
    game_box_im = ImageTk.PhotoImage(game_box_im)
    enemy_im = Image.open("enemy.png")
    enemy_im = ImageTk.PhotoImage(enemy_im)

    # add pieces to graphic
    for box in boxes_in_game.values():
        board.addpiece(str(box), game_box_im, box.row, box.col)
    if hub1 is not None:
        board.addpiece(hub1.message_handle, hub_im, hub1.row, hub1.col)
    for enemy in enemies_in_game:
        board.addpiece(enemy.message_handle, enemy_im, enemy.row, enemy.col)
    for agent in agents_in_game:
        board.addpiece(agent.message_handle, agent_im, agent.row, agent.col)
    ...
    while t < LIMIT_MOVES_PER_GAME and not game_over:
        ...
        if t % BOX_DROP_PERIOD == 0:
            ...
            if len(empty_spots) > 0:
                ...
                box = GameBox(pos[0], pos[1])
                board.addpiece(str(box), game_box_im, pos[0], pos[1])
